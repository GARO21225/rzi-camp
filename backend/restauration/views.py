...
class BonCaisseViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='remboursements_stats')
    def remboursements_stats(self, request):
        """Récupéré du code mort (classe BonCaisseViewSet dupliquée, jamais
        exécutée) avant suppression — stats globales utiles à conserver."""
        from django.utils import timezone
        from django.db.models import Sum
