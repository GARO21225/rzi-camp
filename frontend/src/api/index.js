...
export const personnel = {
  ...
  assignRole: (id, role) => api.patch(`/api/personnel/${id}/assigner_role/`, { role }),
  historiqueVoyages: (id) => api.get(`/api/personnel/${id}/historique_voyages/`),
  importCSV: (file) => {
    const fd = new FormData(); fd.append('file', file)
    return api.post('/api/personnel/import_csv/', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
  },
}
...
export const qr = {
  ...
  viderHistorique: (type) => api.delete(`/api/qr/vider_historique/?type_repas=${type}`),
}
