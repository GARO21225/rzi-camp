    ...
    <div onClick={onClick} style={{ background:bg, borderRadius:14, padding:'16px 18px',
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize:11, color:'var(--rzc-text-3)', fontWeight:600, marginTop:4 }}>{label}</div>
          {sub && <div style={{ fontSize:10, color:'var(--rzc-text-4)', marginTop:2 }}>{sub}</div>}
        </div>
        {trend !== undefined && (
          <span style={{ fontSize:11, fontWeight:700,
            color: trend > 0 ? '#16a34a' : trend < 0 ? '#dc2626' : 'var(--rzc-text-3)',
            background: trend > 0 ? '#dcfce7' : trend < 0 ? '#fee2e2' : 'var(--rzc-charcoal)',
...
function AlerteCard({ type, titre, desc, temps, urgent }) {
  ...
  const c = colors[type] || 'var(--rzc-text-3)'
  return (
    <div style={{ display:'flex', gap:10, padding:'10px 12px', borderRadius:10,
      background: urgent ? '#fef2f2' : 'var(--rzc-charcoal)',
      border:`1.5px solid ${urgent ? '#fecaca' : 'var(--rzc-border-light)'}`,
      marginBottom:6 }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ fontSize:11, color:'var(--rzc-text-3)', marginTop:1 }}>{desc}</div>
      </div>
      <div style={{ fontSize:10, color:'var(--rzc-text-4)', flexShrink:0 }}>{temps}</div>
    </div>
  )
}
  ...
  return (
    <div className="rzc-page-scope" style={{ padding:16 }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:10 }}>
        <div>
          <h1 style={{ fontSize:22, fontWeight:800, color:'var(--rzc-navy)', margin:0 }}>
            🖥️ Centre Opérationnel Temps Réel
          </h1>
          <div style={{ fontSize:12, color:'var(--rzc-text-3)', marginTop:4 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <button onClick={load}
            style={{ background:'var(--rzc-navy)', color:'var(--rzc-white)', border:'none',
              borderRadius:9, padding:'8px 14px', cursor:'pointer', fontSize:12, fontWeight:700 }}>
            🔄 Actualiser
          </button>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        {/* Activité récente */}
        <div style={{ background:'var(--rzc-white)', borderRadius:14, padding:18, boxShadow:'0 2px 8px rgba(0,0,0,.06)' }}>
          <div style={{ fontWeight:800, fontSize:14, color:'var(--rzc-navy)', marginBottom:14 }}>
          ...
          {[
            ...
            { label:'Taux résolution incidents', value:`${inc.resolus||0}/${(inc.declares||0)+(inc.resolus||0)}`, pct: inc.declares ? Math.round(inc.resolus/(inc.declares+inc.resolus)*100) : 100, color:'var(--rzc-navy)' },
            ...
            <div key={label} style={{ marginBottom:14 }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:4 }}>
                <span style={{ color:'var(--rzc-text-3)', fontWeight:600 }}>{label}</span>
                <span style={{ fontWeight:700, color }}>{value}</span>
              </div>
              <div style={{ background:'var(--rzc-charcoal)', borderRadius:99, height:8, overflow:'hidden' }}>
