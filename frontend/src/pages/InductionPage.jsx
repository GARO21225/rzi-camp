...
function exportInductionCSV(list, dateFrom, dateTo) {
  ...
  const rows = filtered.map(p => {
    const rec = p.inductionrecord
    return [p.matricule || '', p.nom || '', p.prenom || '', p.societe || p.entreprise || '',
...
function EtapeFormation({ etape, wf, onValider }) {
  ...
  const _unused_exportInductionCSV = (list, dateFrom, dateTo) => {
    const filtered = list.filter(p => {
      if (!dateFrom && !dateTo) return true
      const rec = p.inductionrecord
      if (!rec?.date_debut) return !dateFrom
