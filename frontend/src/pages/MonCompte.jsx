...
// ─── Config rôles ────────────────────────────────────────────────
const ROLES = {
  admin:        { icon:'👑', label:'Administrateur',   gradient:'linear-gradient(135deg,#1e3a8a,#2563eb)', color:'var(--rzc-navy)' },
...
function PwdField({ label, value, onChange, onEnter }) {
  ...
  return (
    <div style={{ marginBottom:16 }}>
      <label style={{ display:'block', fontSize:11, fontWeight:700, color:'var(--rzc-text-3)', textTransform:'uppercase', letterSpacing:.5, marginBottom:6 }}>
        {label}
      </label>
      <div style={{ position:'relative' }}>
        <input
          ...
          style={{ width:'100%', border:'2px solid #e2e8f0', borderRadius:10, padding:'11px 44px 11px 14px', fontSize:15, outline:'none', fontFamily:'inherit', boxSizing:'border-box', background:'var(--rzc-white)', color:'#1e293b', transition:'border-color .15s' }}
          onFocus={e => e.target.style.borderColor = 'var(--rzc-blue)'}
          onBlur={e => e.target.style.borderColor = 'var(--rzc-border-light)'}
        />
        <button type="button" onClick={() => setShow(s => !s)}
          style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', fontSize:18, color:'var(--rzc-text-4)', padding:0, lineHeight:1 }}>
          {show ? '🙈' : '👁️'}
        </button>
      </div>
    </div>
...
export default function MonCompte() {
  ...
  return (
    <div style={{ padding:20, maxWidth:540, margin:'0 auto' }}>
      ...
      {/* ══ Carte profil ══════════════════════════════════════════ */}
      <div style={{ background: roleInfo.gradient, borderRadius:20, padding:'28px 24px', marginBottom:20, color:'var(--rzc-white)', position:'relative', overflow:'hidden' }}>
        {/* Décorations */}
        ...
        <div style={{ display:'flex', gap:18, alignItems:'center', position:'relative' }}>
          ...
          <div>
            ...
            <div style={{ marginTop:8 }}>
              {pill('rgba(255,255,255,.25)', 'var(--rzc-white)', roleInfo.label)}
            </div>
          </div>
        </div>
      ...
      {/* ══ Tabs ══════════════════════════════════════════════════ */}
      <div style={{ display:'flex', background:'var(--rzc-white)', border:'1px solid #e2e8f0', borderRadius:14, padding:5, marginBottom:20, gap:4 }}>
        {[['profil','👤 Profil'],['securite','🔐 Sécurité']].map(([k,l]) => (
          <button key={k} onClick={() => { setTab(k); setMsg(null) }}
            style={{ flex:1, padding:'10px 0', border:'none', borderRadius:10, cursor:'pointer', fontSize:14, fontWeight:700, transition:'all .15s',
              ...
              color:      tab===k ? 'var(--rzc-white)'         : 'var(--rzc-text-3)',
      ...
      {/* ══ ONGLET PROFIL ══════════════════════════════════════════ */}
      {tab === 'profil' && (
        <div style={{ background:'var(--rzc-white)', border:'1px solid #e2e8f0', borderRadius:16, overflow:'hidden', boxShadow:'0 2px 12px rgba(30,58,138,.07)' }}>
          <div style={{ padding:'14px 20px', borderBottom:'1px solid #f1f5f9', background:'var(--rzc-charcoal)' }}>
            <h3 style={{ fontWeight:700, fontSize:15, color:'var(--rzc-navy)', margin:0 }}>Informations du compte</h3>
          </div>
          <div style={{ padding:20, display:'flex', flexDirection:'column', gap:10 }}>
            {[
              ...
              <div key={label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px', background:'var(--rzc-charcoal)', borderRadius:10, gap:12 }}>
                <span style={{ fontSize:12, color:'var(--rzc-text-3)', fontWeight:600, flexShrink:0 }}>{label}</span>
          ...
          <div style={{ padding:'0 20px 20px' }}>
            <button onClick={() => setTab('securite')}
              style={{ width:'100%', background: roleInfo.color, color:'var(--rzc-white)', border:'none', padding:13, borderRadius:12, cursor:'pointer', fontSize:14, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
      ...
      {tab === 'securite' && (
        <div style={{ background:'var(--rzc-white)', border:'1px solid #e2e8f0', borderRadius:16, overflow:'hidden', boxShadow:'0 2px 12px rgba(30,58,138,.07)' }}>
          <div style={{ padding:'14px 20px', borderBottom:'1px solid #f1f5f9', background:'var(--rzc-charcoal)' }}>
            <h3 style={{ fontWeight:700, fontSize:15, color:'var(--rzc-navy)', margin:0 }}>🔐 Modifier le mot de passe</h3>
            <p style={{ fontSize:12, color:'var(--rzc-text-3)', margin:'4px 0 0' }}>Vous serez déconnecté automatiquement après la modification.</p>
          </div>
          <div style={{ padding:20 }}>
            ...
            {nouveau && (
              <div style={{ marginTop:-8, marginBottom:16 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
                  <span style={{ fontSize:11, color:'var(--rzc-text-3)' }}>Force du mot de passe</span>
                  <span style={{ fontSize:11, fontWeight:700, color: strengthInfo?.color }}>
                    {strengthInfo?.emoji} {strengthInfo?.label}
                  </span>
                </div>
                <div style={{ height:5, background:'var(--rzc-charcoal)', borderRadius:10, overflow:'hidden' }}>
            ...
            <button
              ...
              style={{ width:'100%', background: loading || !ancien || !nouveau || !confirmer || nouveau !== confirmer ? 'var(--rzc-text-4)' : roleInfo.color, color:'var(--rzc-white)', border:'none', padding:14, borderRadius:12, cursor: loading ? 'wait' : (!ancien || !nouveau || !confirmer ? 'not-allowed' : 'pointer'), fontSize:15, fontWeight:700, transition:'.15s', marginTop:4 }}>
